import{ZoomBehavior}from"chrome-extension://mhjfbmdgcfjbbpaeojofohoefgiehjai/browser_api.js";export{BrowserApi,ZoomBehavior}from"chrome-extension://mhjfbmdgcfjbbpaeojofohoefgiehjai/browser_api.js";import{isIOS,isWindows,isMac}from"chrome://resources/js/cr.m.js";import{html,Polymer,Base,dom,PolymerElement}from"chrome://resources/polymer/v3_0/polymer/polymer_bundled.min.js";import{loadTimeData}from"chrome://resources/js/load_time_data.m.js";// Copyright 2017 The Chromium Authors. All rights reserved.
const FittingType={NONE:"none",FIT_TO_PAGE:"fit-to-page",FIT_TO_WIDTH:"fit-to-width",FIT_TO_HEIGHT:"fit-to-height"};const SaveRequestType={ANNOTATION:0,ORIGINAL:1,EDITED:2};let Point;// Copyright (c) 2013 The Chromium Authors. All rights reserved.
 ...
 */const template$9=html`<custom-style>
...
</custom-style>`;template$8.setAttribute("style","display: none;");document.head.appendChild(template$8.content);var style=document.createElement("style");style.textContent="[hidden] { display: none !important; }";document.head.appendChild(style);
/**
 @license
 Copyright (c) 2015 The Polymer Project Authors. All rights reserved.
 This code may only be used under the BSD style license found at
...
const template$6=document.createElement("template");template$6.innerHTML=`<dom-module id="pdf-shared">\x3c!--_html_template_start_--\x3e\n<template>\n  <style scope="pdf-shared">cr-icon-button {\n  --cr-icon-button-fill-color: var(--pdf-toolbar-text-color);\n      --cr-icon-button-focus-outline-color: var(--google-grey-500);\n      margin: 0;\n}\n\ncr-icon-button:hover {\n  background: rgba(255, 255, 255, 0.08);\n      border-radius: 50%;\n}\n\ncr-action-menu, viewer-bookmark {\n  --cr-menu-background-color: var(--google-grey-900);\n      --cr-menu-shadow: rgba(0, 0, 0, .3) 0 1px 2px 0,\n                        rgba(0, 0, 0, .15) 0 3px 6px 2px;\n      --cr-primary-text-color: var(--google-grey-200);\n      --cr-menu-background-focus-color: var(--google-grey-700);\n      --cr-menu-background-sheen: rgba(255, 255, 255, .06);\n      --cr-separator-line: var(--cr-separator-height) solid\n          rgba(255, 255, 255, .1);\n}\n\n</style>\n</template>\n\x3c!--_html_template_end_--\x3e</dom-module>\n`;document.body.appendChild(template$6.content.cloneNode(true));// Copyright 2015 The Chromium Authors. All rights reserved.
const BOOKMARK_INDENT=20;class ViewerBookmarkElement extends PolymerElement{static get is(){return"viewer-bookmark"}static get template(){return html`<!--css-build:shadow--><!--_html_template_start_--><style include="pdf-shared" scope="viewer-bookmark">#item {
  ...
  transform: rotate(90deg);
    ...
    <div id="item" on-click="onClick_">
      <div id="expand-container">
        <cr-icon-button id="expand" iron-icon="cr:chevron-right" aria-label="Раздел" aria-expanded$="[[getAriaExpanded_(childrenShown_)]]" on-click="toggleChildren_"></cr-icon-button>
      </div>
...
<!--_html_template_end_-->`}static get properties(){return{bookmark:{type:Object,observer:"bookmarkChanged_"},depth:{type:Number,observer:"depthChanged_"},childDepth_:Number,childrenShown_:{type:Boolean,reflectToAttribute:true,value:false}}}ready(){super.ready();this.$.item.addEventListener("keydown",(e=>{const keyboardEvent=e;if(keyboardEvent.key==="Enter"){this.onEnter_(keyboardEvent)}else if(keyboardEvent.key===" "){this.onSpace_(keyboardEvent)}}))}fire_(eventName,detail){this.dispatchEvent(new CustomEvent(eventName,{bubbles:true,composed:true,detail:detail}))}bookmarkChanged_(){this.$.expand.style.visibility=this.bookmark.children.length>0?"visible":"hidden"}depthChanged_(){this.childDepth_=this.depth+1;this.$.item.style.paddingInlineStart=this.depth*BOOKMARK_INDENT+"px"}onClick_(){if(this.bookmark.page!=null){if(this.bookmark.zoom!=null){this.fire_("change-zoom",{zoom:this.bookmark.zoom})}if(this.bookmark.x!=null&&this.bookmark.y!=null){this.fire_("change-page-and-xy",{page:this.bookmark.page,x:this.bookmark.x,y:this.bookmark.y,origin:"bookmark"})}else{this.fire_("change-page",{page:this.bookmark.page,origin:"bookmark"})}}else if(this.bookmark.uri!=null){this.fire_("navigate",{uri:this.bookmark.uri,newtab:true})}}onEnter_(e){if(e.target!==this.$.expand){this.onClick_()}}onSpace_(e){this.onClick_();e.preventDefault()}toggleChildren_(e){this.childrenShown_=!this.childrenShown_;e.stopPropagation()}getAriaExpanded_(){return this.childrenShown_?"true":"false"}}customElements.define(ViewerBookmarkElement.is,ViewerBookmarkElement);// Copyright 2020 The Chromium Authors. All rights reserved.
...
const template$5=html`<!--_html_template_start_--><iron-iconset-svg size="24" name="pdf">
  <svg>
    <defs>
      <!--
      These icons are copied from Polymer's iron-icons and kept in sorted order.
...
<!--_html_template_end_-->`;document.head.appendChild(template$5.content);// Copyright (c) 2012 The Chromium Authors. All rights reserved.
// Use of this source code is governed by a BSD-style license that can be
// found in the LICENSE file.
...
<cr-icon-button id="download" iron-icon="cr:file-download" on-click="onDownloadClick_" aria-label="Скачать" aria-haspopup$="[[downloadHasPopup_]]" title="Скачать"></cr-icon-button>
...
const $_documentContainer=document.createElement("template");$_documentContainer.innerHTML=`\x3c!--_html_template_start_--\x3e<custom-style>\n  <style css-build="shadow">html {\n  --iron-icon-height: 20px;\n      --iron-icon-width: 20px;\n      --viewer-icon-ink-color: rgb(189, 189, 189);\n      --viewer-pdf-toolbar-background-color: rgb(50, 54, 57);\n      --viewer-text-input-selection-color: rgba(255, 255, 255, 0.3);\n}\n\n</style>\n</custom-style>\n\x3c!--_html_template_end_--\x3e`;document.head.appendChild($_documentContainer.content);// Copyright 2015 The Chromium Authors. All rights reserved.
...
#divider {
  margin: 0 var(--page-selector-spacing);
    ...
    <div id="content">
      <input part="input" type="text" id="pageselector" value="[[pageNo]]" on-pointerup="select" on-input="onInput_" on-change="pageNoCommitted" aria-label="Номер страницы">
...
class ViewerPasswordDialogElement extends PolymerElement{static get is(){return"viewer-password-dialog"}static get template(){return html`<!--css-build:shadow--><!--_html_template_start_--><style include="cr-shared-style" scope="viewer-password-dialog">#password {
  margin-top: var(--cr-form-field-bottom-spacing);
    ...
    <cr-dialog id="dialog" no-cancel="" show-on-attach="">
      <div slot="title">Необходимо ввести пароль</div>
      <div slot="body">
        <div id="message">Документ защищен паролем. Введите пароль.</div>
        <cr-input id="password" type="password" error-message="Неправильный пароль" invalid="[[invalid]]" autofocus="">
        </cr-input>
      </div>
...
<div id="icons" hidden$="[[!bookmarks.length]]">
  <div class$="
    ...
    <cr-icon-button iron-icon="pdf:thumbnails" role="tab" title="Уменьшенные изображения" aria-label="Уменьшенные изображения" aria-selected$="[[getAriaSelectedThumbnails_(thumbnailView_)]]" on-click="onThumbnailClick_">
    </cr-icon-button>
  </div>
  <div class$="
      button-wrapper cr-vertical-tab [[outlineButtonClass_(thumbnailView_)]]">
    <cr-icon-button iron-icon="pdf:doc-outline" role="tab" title="Структура документа" aria-label="Структура документа" aria-selected$="[[getAriaSelectedOutline_(thumbnailView_)]]" on-click="onOutlineClick_">
    </cr-icon-button>
  </div>
</div>
<div id="content">
...
<cr-dialog show-on-attach="">
  <div slot="title">Свойства документа</div>
  <div slot="body">
    <table>
      <tbody><tr>
        <td class="name">Имя файла:</td>
        <td class="value" id="file-name">[[fileName]]</td>
      </tr>
      <tr class="break">
        <td class="name">Размер файла:</td>
...
class ViewerToolbarElement extends PolymerElement{static get is(){return"viewer-toolbar"}static get template(){return html`<!--css-build:shadow--><!--_html_template_start_--><style include="pdf-shared" scope="viewer-toolbar">:host {
...
<div id="toolbar">
  <div id="start">
    <cr-icon-button id="sidenavToggle" iron-icon="cr20:menu" title="Меню" aria-label="Меню" aria-expanded$="[[getAriaExpanded_(sidenavCollapsed)]]" on-click="onSidenavToggleClick_">
    </cr-icon-button>
  ...
  <div id="center">
    ...
    <span id="zoom-controls">
      <cr-icon-button iron-icon="pdf:remove" title="Уменьшить" disabled="[[isAtMinimumZoom_(zoomBounds.min, viewportZoomPercent_)]]" aria-label="Уменьшить" on-click="onZoomOutClick_">
      </cr-icon-button>
      <input type="text" value="100%" aria-label="Масштаб" on-change="onZoomChange_" on-pointerup="onZoomInputPointerup_" on-blur="onZoomChange_">
      
      <cr-icon-button iron-icon="pdf:add" title="Увеличить" disabled="[[isAtMaximumZoom_(zoomBounds.max, viewportZoomPercent_)]]" aria-label="Увеличить" on-click="onZoomInClick_">
      </cr-icon-button>
    </span>
    <span class="vertical-separator"></span>
    <cr-icon-button id="fit" iron-icon="[[fitToButtonIcon_]]" title="[[getFitToButtonTooltip_('По размеру страницы',
                                        'Выровнять по ширине окна',
                                        fittingType_)]]" aria-label="[[getFitToButtonTooltip_('По размеру страницы',
                                             'Выровнять по ширине окна',
                                             fittingType_)]]" on-click="onFitToButtonClick_">
    </cr-icon-button>
    <cr-icon-button iron-icon="pdf:rotate-left" dir="ltr" aria-label="Повернуть против часовой стрелки" title="Повернуть против часовой стрелки" on-click="onRotateClick_">
    </cr-icon-button>
  </div>
  <div id="end">
    ...
    <cr-icon-button id="print" iron-icon="cr:print" hidden="[[!printingEnabled]]" title="Печать" aria-label="Печать" on-click="onPrintClick_">
    </cr-icon-button>
    <cr-icon-button id="more" iron-icon="cr:more-vert" title="Другие действия" aria-label="Другие действия" on-click="onMoreClick_"></cr-icon-button>
  </div>
</div>
