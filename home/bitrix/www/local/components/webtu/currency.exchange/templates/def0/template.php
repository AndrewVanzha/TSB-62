                ...
                <? if(isset($arResult['COURSES']['tsb'][$arOffice['PROPERTY_ATT_CODE_VALUE']])) : ?>
                    <div class="currency-table--item">
                        ...
                        <div class="currency-table--item__exchange">
                            ...
                            <div class="currency-table--item__exchange_grid currency-table--item__exchange_gridrow <?= (isset($arResult['COURSES']['tsb'][$arOffice['PROPERTY_ATT_CODE_VALUE']]['currency'][$currencyCode][1]))? 'currency-table--item__exchange_widerow_1' : ''; ?>">
                            ...
                            <? if (isset($arResult['COURSES']['tsb'][$arOffice['PROPERTY_ATT_CODE_VALUE']]['currency'][$currencyCode][1])) : ?>
                                <div class="currency-table--item__exchange_grid currency-table--item__exchange_gridrow currency-table--item__exchange_widerow_2">
    ...
    $(document).ready(function() {
        ...
        function calculateCurrValue(curr_array, operation_type, initial_sum) {
            ...
            if(operation_type == 'buy') {
                curr_value = curr_array[0].sell;
                if(parseInt(initial_sum) >= parseInt(curr_volume)) {
                    if(curr_array[1]) {
                        curr_value = curr_array[1].sell;
                    }
                }
            } else {
                curr_value = curr_array[0].buy;
                if(parseInt(initial_sum) >= parseInt(curr_volume)) {
                    if(curr_array[1]) {
                        curr_value = curr_array[1].buy;
                    }
                }
            }
            return curr_value;
        ...
        let findOfficeCurrencyRate = id => { // возвращает объект с массивом валюты для выбранного офиса
        ...
        [].forEach.call(leftInputs, function (elem) { // перебираю все изменения левого input
            ...
            elem.addEventListener("input", (event) => {
                ...
                //console.log('initialLeftSum='+initialLeftSum);
                ...
                //let curr_array = Object.values(curr_obj)[0];
                //console.log(curr_array);
