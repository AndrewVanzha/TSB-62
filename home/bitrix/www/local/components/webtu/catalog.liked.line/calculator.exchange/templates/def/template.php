...
<form action="" method="get" class="exchange-converter">
    ...
    <div class="note">
        По курсу <span id='val-cur'><?=$curVar?></span> руб.
	    <?if ($arResult['NAME_OFFICE'] != 'iSimple') {
		    $officeName = $arResult['NAME_OFFICE'];
	    } else {
		    $officeName = 'ТСБ-онлайн';
	    }?>
        <p>Данные для офиса «<?=$officeName;?>» по состоянию на <span id="exchange-date"></span></p>
        <p>Просим дополнительно ознакомиться с <a href="https://www.transstroybank.ru/chastnym-klientam/drugie-uslugi/raschetno-kassovoe-obsluzhivanie">"Тарифами на проведение валютно-обменных операций"</a></p>
		<?php if(empty($_SESSION["city"]) || $_SESSION["city"] == 399): ?>
		<a href="/chastnym-klientam/rezervirovanie-kursa/"
			class="button" 
			style="display:inline-block;width:auto"
		>Зарезервировать курс валюты</a>
		<?php endif; ?>
    </div>
