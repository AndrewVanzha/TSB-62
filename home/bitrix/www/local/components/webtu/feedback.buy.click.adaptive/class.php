...
class FeedbackBuyClickAdaptive extends CBitrixComponent
{
    ...
    protected function checkModules()
    {
        if (!Loader::includeModule('iblock')){
            throw new Main\LoaderException(Loc::getMessage('FBKA_NOT_INSTALLED_IBLOCK', Array ("#ID#" => "iblock")) );
        }
    }
    ...
    protected function getPropertyRequired()
    {
        ...
        foreach($arResult as $item){
            if( $this->arResult["REQUEST"]["PROP"][$item] == "" ){
                array_push($this->MessageError, Loc::getMessage('FBKA_ERROR_'.$item) );
            }
        }
    ...
    protected function getResult()
    {
        ...
        $this->arResult["REQUEST"]["FBKA_SUBMIT"] = $request->getPost("FBKA_SUBMIT");

        if($request->isPost() && $this->arResult["REQUEST"]["FBKA_SUBMIT"] && $this->arResult["REQUEST"]["FBKA_SUBMIT"] != "" ){
            #CAPTCHA
                ...
                if (file_exists($path)) {
        			...
        			if (strlen($captcha_word) > 0 && strlen($captcha_code) > 0){
        				if (!$cpt->CheckCodeCrypt($captcha_word, $captcha_code, $captchaPass)){
                            array_push($this->MessageError, Loc::getMessage('FBKA_CAPTCHA_WRONG') );   
        				}
        			}else{
                        array_push($this->MessageError, Loc::getMessage('FBKA_CAPTHCA_EMPTYE') );              
        			}
                }
            }
            ...
            if(count($this->MessageError) == 0){
                ...
                if($PRODUCT_ID = $el->Add($arLoadProductArray)){
                    /*array_push($this->MessageSend, Loc::getMessage('FBKA_SEND_SAVE') );*/
                    ...
                    $result = CEvent::Send('FEEDBACK_BUY_CLICK_2', SITE_ID, $arMailFields);  

                    if($result){
                        array_push($this->MessageSend, Loc::getMessage('FBKA_SEND_DONE') );
                        ...
                        array_push($this->MessageError, Loc::getMessage('FBKA_ERROR_SEND_ELEMENT') );
                    }

                }else{
                    array_push($this->MessageError, Loc::getMessage('FBKA_ERROR_ADD_ELEMENT') );   
                }
            }
        }
    ...
    {
		try{
            $this->arResult["COMPONENT_ID"] = 'FBCA';
