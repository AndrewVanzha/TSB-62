...
class FeedbackSubscriptionProduct extends CBitrixComponent
{
    ...
    protected function checkModules()
    {
        if (!Loader::includeModule('iblock')){
            throw new Main\LoaderException(Loc::getMessage('FSP_NOT_INSTALLED_IBLOCK', Array ("#ID#" => "iblock")) );
        }
    }
    ...
    protected function getPropertyRequired()
    {
        ...
        foreach($arResult as $item){
            if( $this->arResult["REQUEST"]["PROP"][$item] == "" ){
                array_push($this->MessageError, Loc::getMessage('FSP_ERROR_'.$item) );
            }
        }
    ...
    protected function getResult()
    {
        ...
        $this->arResult["REQUEST"]["FSP_SUBMIT"] = $request->getPost("FSP_SUBMIT");

        #ID товара из input (для списков товаров)
        if ($this->arResult["REQUEST"]["PROP"]["PRODUCT_ID"] > 0) {
            $this->arParams["PRODUCT_ID"] = $this->arResult["REQUEST"]["PROP"]["PRODUCT_ID"];
        }
        ...
        if($request->isPost() && $this->arResult["REQUEST"]["FSP_SUBMIT"] && $this->arResult["REQUEST"]["FSP_SUBMIT"] != "" ){
            #CAPTCHA
                ...
                if (file_exists($path)) {
        			...
        			if (strlen($captcha_word) > 0 && strlen($captcha_code) > 0){
        				if (!$cpt->CheckCodeCrypt($captcha_word, $captcha_code, $captchaPass)){
                            array_push($this->MessageError, Loc::getMessage('FSP_CAPTCHA_WRONG') );   
        				}
        			}else{
                        array_push($this->MessageError, Loc::getMessage('FSP_CAPTHCA_EMPTYE') );              
        			}
                }
            }
            ...
            if(count($this->MessageError) == 0){
                ...
                if($PRODUCT_ID = $el->Add($arLoadProductArray)){
                   /* array_push($this->MessageSend, Loc::getMessage('FSP_SEND_SAVE') );*/
                    ...
                    $result = CEvent::Send('FEEDBACK_SUBSCRIPTION_PRODUCT', SITE_ID, $arMailFields);  

                    if($result){
                        array_push($this->MessageSend, Loc::getMessage('FSP_SEND_DONE') );
                        ...
                        array_push($this->MessageError, Loc::getMessage('FSP_ERROR_SEND_ELEMENT') );
                    }

                }else{
                    array_push($this->MessageError, Loc::getMessage('FSP_ERROR_ADD_ELEMENT') );   
                }
            }
        }
    ...
    {
		try{
            $this->arResult["COMPONENT_ID"] = 'FSP';
