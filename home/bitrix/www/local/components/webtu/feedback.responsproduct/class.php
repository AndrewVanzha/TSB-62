...
class FeedbackResponsProduct extends CBitrixComponent
{
    ...
    protected function checkModules()
    {
        if (!Loader::includeModule('iblock')){
            throw new Main\LoaderException(Loc::getMessage('FRP_NOT_INSTALLED_IBLOCK', Array ("#ID#" => "iblock")) );
        }
    }
    ...
    {
        $arResult = array("USER_NAME","RESPONSE");

        foreach($arResult as $item){
            if( $this->arResult["REQUEST"]["PROP"][$item] == "" ){
                array_push($this->MessageError, Loc::getMessage('FRP_ERROR_'.$item) );
            }
        }
    ...
    protected function getResult()
    {
        ...
        $this->arResult["REQUEST"]["FRP_SUBMIT"] = $request->getPost("FRP_SUBMIT");

        if($request->isPost() && $this->arResult["REQUEST"]["FRP_SUBMIT"] && $this->arResult["REQUEST"]["FRP_SUBMIT"] != "" ){
            #CAPTCHA
                ...
                if (file_exists($path)) {
        			...
        			if (strlen($captcha_word) > 0 && strlen($captcha_code) > 0){
        				if (!$cpt->CheckCodeCrypt($captcha_word, $captcha_code, $captchaPass)){
                            array_push($this->MessageError, Loc::getMessage('FRP_CAPTCHA_WRONG') );   
        				}
        			}else{
                        array_push($this->MessageError, Loc::getMessage('FRP_CAPTHCA_EMPTYE') );              
        			}
                }
            }
            ...
            if(count($this->MessageError) == 0){
                #Назваие в транслите
                $title = "Отзыв на товара ID=".$this->arResult["PRODUCT"]["ID"]. " (".$this->arResult["PRODUCT"]["NAME"].")";
                ...
                $arLoadProductArray = Array(
                    ...
                    "CODE"               => $trans.'-'.mktime(),
                	"ACTIVE"             => "N",
                	"PREVIEW_TEXT"       => $this->arResult["REQUEST"]["PROP"]["RESPONSE"],
                	"DETAIL_TEXT"	     => "",
                ...
                if($PRODUCT_ID = $el->Add($arLoadProductArray)){
                   ...
                   /* array_push($this->MessageSend, Loc::getMessage('FRP_SEND_SAVE') );*/
                    
                    #Отправляем письмо админу           
                    $arMailFields = array(
                        "PRODUCT_ID"     => $this->arResult["PRODUCT"]["ID"],
                        "USER_NAME"      => $this->arResult["REQUEST"]["PROP"]["USER_NAME"],
                        "RESPONSE"    => $this->arResult["REQUEST"]["PROP"]["RESPONSE"],
                    ...
                    if($result){
                        array_push($this->MessageSend, Loc::getMessage('FRP_SEND_DONE') );
                        ...
                        array_push($this->MessageError, Loc::getMessage('FRP_ERROR_SEND_ELEMENT') );
                    }

                }else{
                    array_push($this->MessageError, Loc::getMessage('FRP_ERROR_ADD_ELEMENT') );   
                }
    ...
    {
		try{
            $this->arResult["COMPONENT_ID"] = 'FRP';
