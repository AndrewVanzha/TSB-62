$(document).ready(function () {
    ...
    $('#brokerageOrder').submit(function (e) {
        e.preventDefault();
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        if (data.message && data.message.length > 0) {
                            $(".v21_alert_brokerageOrder_item").remove()
                            $.each(data.message, function (key, field) {
                                $('#v21_alert_brokerageOrder .v21-modal__window').append(
                                    '<div class="v21-grid__item v21_alert_brokerageOrder_item" style="font-size: 20px; padding: 0; text-align: center;">' + field.text + '</div>'
                                );

                                if (!field.type) {
                                    $('.v21_alert_brokerageOrder_item').css("color", "red");
                                }
                            });
                        }
                        if (data.status) {
                            $("#brokerageOrder")[0].reset();
                        }
                        ...
                        if (!data.captcha){
                            ...
                            tsb21.modal.toggleModal('v21_alert_brokerageOrder');
                        }
                    }
                });
            }
    ...
    $('a[href="#v21_brokerageOrder"].open').click(function () {
