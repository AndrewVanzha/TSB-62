...
class FeedbackBuyClick extends CBitrixComponent
{
    ...
    protected function checkModules()
    {
        if (!Loader::includeModule('iblock')){
            throw new Main\LoaderException(Loc::getMessage('FBK_NOT_INSTALLED_IBLOCK', Array ("#ID#" => "iblock")) );
        }
    }
    ...
    protected function getPropertyRequired()
    {
        ...
        foreach($arResult as $item){
            if( $this->arResult["REQUEST"]["PROP"][$item] == "" ){
                array_push($this->MessageError, Loc::getMessage('FBK_ERROR_'.$item) );
            }
        }
    ...
    protected function getResult()
    {
        ...
        $this->arResult["REQUEST"]["FBK_SUBMIT"] = $request->getPost("FBK_SUBMIT");

        if($request->isPost() && $this->arResult["REQUEST"]["FBK_SUBMIT"] && $this->arResult["REQUEST"]["FBK_SUBMIT"] != "" ){
            #CAPTCHA
                ...
                if (file_exists($path)) {
        			...
        			if (strlen($captcha_word) > 0 && strlen($captcha_code) > 0){
        				if (!$cpt->CheckCodeCrypt($captcha_word, $captcha_code, $captchaPass)){
                            array_push($this->MessageError, Loc::getMessage('FBK_CAPTCHA_WRONG') );   
        				}
        			}else{
                        array_push($this->MessageError, Loc::getMessage('FBK_CAPTHCA_EMPTYE') );              
        			}
                }
            }
            ...
            if(count($this->MessageError) == 0){
                ...
                if($PRODUCT_ID = $el->Add($arLoadProductArray)){
                    /*array_push($this->MessageSend, Loc::getMessage('FBK_SEND_SAVE') );*/
                    ...
                    $result = CEvent::Send('FEEDBACK_BUY_CLICK', SITE_ID, $arMailFields);  

                    if($result){
                        array_push($this->MessageSend, Loc::getMessage('FBK_SEND_DONE') );
                        ...
                        array_push($this->MessageError, Loc::getMessage('FBK_ERROR_SEND_ELEMENT') );
                    }

                }else{
                    array_push($this->MessageError, Loc::getMessage('FBK_ERROR_ADD_ELEMENT') );   
                }
            }
        }
    ...
    {
		try{
            $this->arResult["COMPONENT_ID"] = 'FBC';
