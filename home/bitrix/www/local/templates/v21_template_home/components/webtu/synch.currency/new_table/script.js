window.addEventListener('DOMContentLoaded', function () {
    ...
    function notesEventListeners() {
        ...
        [].forEach.call(check_notes_text, function (elem) { // клик на каждую сноску
            console.log(elem);
            $(elem).find('.exchange-table__text-subline').toggle('exchange-table__text-subline--show');
            elem.addEventListener('click', function (event) {
                event.path[1].nextElementSibling.classList.toggle('exchange-table__text-subline--show');
                ...
                if(event.path[1].nextElementSibling.children[1]) {
                    event.path[1].nextElementSibling.children[1].addEventListener('click', function (ev) {
                        //console.log(ev.path);
                        ev.path[2].classList.remove('exchange-table__text-subline--show');
                    });
                }
            });
        });*/
    ...
    /*function checkWorkHoursPeriod() {
        ...
        if(exchange_calc_button) {
            if(wStart && wEnd && wDay) {
                exchange_calc_button.classList.remove('exchange-calc__button-disable');
            } else {
                exchange_calc_button.classList.add('exchange-calc__button-disable');
            }
        }
    }*/
    ...
    /*function displayTime() {
        ...
        let url = '/chastnym-klientam/konvertor-valyut/ajax_currency_table.php' + '?office=' + office;
