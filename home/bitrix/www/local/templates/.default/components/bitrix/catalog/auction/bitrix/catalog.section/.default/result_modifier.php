...
if (!empty($arResult['ITEMS']))
{
	...
	foreach ($arResult['ITEMS'] as $key => $arItem)
	{
		...
		if (empty($productPictures['SECOND_PICT'])){
            if(defined('UF_SECTION_IMAGE')){
                $productPictures['SECOND_PICT']['SRC'] = UF_SECTION_IMAGE;
            }else{
...
if (!Bitrix\Main\Loader::includeModule("webtu.auction")){
    return;
}

$arOptions = Handler::getOptions();

$arResult["MODULE_SETTINGS"] = $arOptions;
unset($arOptions);
