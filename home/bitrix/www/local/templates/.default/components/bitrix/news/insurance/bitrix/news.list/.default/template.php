...
<section class="page-section">
    <div class="product-items clearfix">
        ...
        <? foreach($arResult["ITEMS"] as $arItem) { ?>
        	...
        	<div data-type='<?=json_encode($arItem['PROPERTIES']['TYPE']['VALUE'])?>' data-pay="<?=$arItem['PROPERTIES']['PAY_SYSTEM']['VALUE']?>" data-view="<?=$arItem['PROPERTIES']['VIEW']['VALUE']?>" class="product-item <?if(++$i > 4) echo 'hidden';?>" id="item">
                <?$arr = array("Дебетовая карта", "Зарплатная карта");?>
                <input type="hidden" data-type='<?=json_encode($arr);?>' class="test">
