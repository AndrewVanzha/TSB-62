...
<div class="search-page">
	<form action="" method="get">
		<div class="search-results_bar clearfix">
			...
			<? else : ?>
				<input type="search" name="q" id="search_query" class="input-field" value="<?= $arResult["REQUEST"]["QUERY"] ?>" />
			<? endif; ?>
			...
			<? endif; ?>
			<button type="submit" class="button" value="<?= GetMessage("SEARCH_GO") ?>"><span class="mi--zoom-3 mi">Искать</span></button>
	...
	<? elseif (count($arResult["SEARCH"]) > 0) : ?>
		...
		<div class="search-results_query">
			Результаты поиска по запросу <span>«<?= $arResult["REQUEST"]["QUERY"] ?>»</span>: найдено&nbsp;<?= $arResult["NAV_RESULT"]->NavRecordCount ?>
		</div>

		<?
		$selected = "selected";
		$arrayResults = $arResult["SEARCH"];
		if ($_REQUEST["sort"] == "desc")
			$arrayResults = array_reverse($arResult["SEARCH"], true);
		?>

		<div class="search-results_sorting clearfix">
			<div class="caption">
				Отсортировать:
			</div>
			<div class="select-box">
				<select name="">
					...
					<option value="" <? if ($arResult["REQUEST"]["HOW"] == "d") echo $selected; ?>>
						По дате (сначала новые)
					</option>

					<option value="" <? if ($arResult["REQUEST"]["HOW"] == "d" && $_REQUEST["sort"] == "desc") echo $selected; ?>>
		...
		<?/*
				<p>
					...
					<? endif; ?>
						...
						*/ ?>
		<? foreach ($arrayResults as $arItem) : ?>

			<?
			$grab = GetIBlockElement($arItem["ITEM_ID"]);
			$image = CFile::GetPath($grab["DETAIL_PICTURE"]);
			?>

			<section class="content-area page-section clearfix">

				<img src="<?= $image ?>" alt="" class="content-area_image">
				...
				<div class="content-area_text">
					...
					<div class="search-results_category">
						Раздел:
						<a href="<?= $grab["LIST_PAGE_URL"] ?>">
							<?= $grab["IBLOCK_NAME"] ?>
						</a>
					</div>
