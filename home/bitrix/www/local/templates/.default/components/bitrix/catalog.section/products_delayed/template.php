...
$arWords = array("товар", "товара", "товаров");

if (file_exists($_SERVER['DOCUMENT_ROOT'] . "/local/include/countryList.php"))
    include $_SERVER['DOCUMENT_ROOT'] . '/local/include/countryList.php';
        ...
        <div class="block clearfix">
            <div class="title">
                ...
                <div class="number aligner">
                    <?=( ( count($arResult['ITEMS']) == 1 ) ? '' :  '' ).count($arResult['ITEMS']).' '.getWord(count($arResult['ITEMS']), $arWords)?>
                </div>
            </div>
            <div class="controls-wrap clearfix">
                <? if(count($arResult["SORTS"]) > 0) { ?>
                    <div class="size size-1">
                        <select name="sort-product" id="sort-product">
                            <? foreach ($arResult["SORTS"] as $key => $sort) {
    ...
    <div class="product-wrap">
        <div class="row">
            ...
            foreach($arResult['ITEMS'] as $key => $arItem) {
                ...
                ?>
                <div class="product-box <?=$_COOKIE['view']?>" id="favorite-<?=$arItem['ID']?>">
                            ...
                            <div class="block">
                                ...
                                <? if ($arItem["IBLOCK_SECTION_ID"] == 18) { ?>
                                    <div class="info-hover-text">
                                        Размен возможен по адресу г.&nbsp;Москва, Дубининская д. 94
                                    </div>
                                <? } elseif ($price > 0 && $arItem["PROPERTIES"]["AVAILABLE"]["VALUE"] == 1) { ?>
                                    <div class="product-btn clearfix">
                                        <div class="left">
                                            <?
                                            /** --- Избранное --- **/
                                            echo '<a href="javascript:void(0);" onclick="list_deleted_liked(this,'.$arItem["ID"].')" class="favorite is-active" title="Удалить из отложенного"></a>';
                                            ?>
                                            <div class="count">
